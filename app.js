...
const API_CHANGE_PW = '/api/auth/change-password';
...
const settingMitigation = document.getElementById('setting-mitigation');
...
let secondsLeft = 0;
let systemSettings = { mitigation_enabled: true, poll_interval: 2000, sound_enabled: true };
let currentActiveTab = 'timeline';

// Live timing and synth variables
let syncLoopInterval = null;
let alarmAudioContext = null;
let alarmOscillator = null;
let alarmModulationInterval = null;
...
function playSecurityAlarm() {
    if (!systemSettings.sound_enabled) {
        stopSecurityAlarm();
        return;
    }
    try {
        ...
        osc.type = 'sawtooth';
        osc.frequency.setValueAtTime(440, alarmAudioContext.currentTime);
        
        let toggle = true;
        alarmModulationInterval = setInterval(() => {
            if (!alarmAudioContext || alarmAudioContext.state === 'closed') return;
            const t = alarmAudioContext.currentTime;
            osc.frequency.linearRampToValueAtTime(toggle ? 880 : 440, t + 0.4);
            toggle = !toggle;
        }, 500);
...
function stopSecurityAlarm() {
    if (alarmModulationInterval) {
        clearInterval(alarmModulationInterval);
        alarmModulationInterval = null;
    }
...
function switchTab(tabName) {
    ...
    const tabTimeline = document.getElementById('tab-content-timeline');
    ...
    if (tabName === 'timeline') {
        tabTimeline.style.display = 'block';
        tabSettings.style.display = 'none';
        btnTimeline.className = 'btn btn-whitelist';
        btnSettings.className = 'btn';
        fetchUsbData();
    } else {
        tabTimeline.style.display = 'none';
        tabSettings.style.display = 'block';
        btnTimeline.className = 'btn';
        btnSettings.className = 'btn btn-whitelist';
        loadSystemSettings();
    }
}
...
async function loadSystemSettings() {
    try {
        ...
        if (response.ok) {
            ...
            settingMitigation.checked = systemSettings.mitigation_enabled;
            settingSound.checked = systemSettings.sound_enabled;
            settingInterval.value = systemSettings.poll_interval.toString();
            resetPollingLoop(systemSettings.poll_interval);
        }
    } catch(e) {
        console.error(e);
    }
...
async function saveSystemSettings() {
    const mitigation = settingMitigation.checked;
    const sound = settingSound.checked;
    const interval = parseInt(settingInterval.value);
    ...
    try {
        ...
        if (response.ok) {
            systemSettings = { mitigation_enabled: mitigation, poll_interval: interval, sound_enabled: sound };
            resetPollingLoop(interval);
...
async function changeMasterPassword() {
    const oldPw = changeOldPassword.value;
    const newPw = changeNewPassword.value;
    
    if (!oldPw || !newPw || newPw.length < 4) {
        alert('All fields are required. New password must be at least 4 characters long.');
        return;
    }
    
    try {
        const response = await fetch(API_CHANGE_PW, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
            ...
            alert('Master credentials successfully updated!');
        } else {
            alert('Password update failed. Verify current old password.');
        }
    } catch(e) {
        console.error(e);
        alert('Network error.');
