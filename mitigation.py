...
def disable_unauthorized_device(serial, is_usbstor=True):
    ...
    cmd = f"Get-PnpDevice | Where-Object {{ $_.InstanceId -like '*{escaped_serial}*' }} | Disable-PnpDevice -Confirm:$false"
    
    print("DEBUG: Executing mitigation command for serial: " + serial)
    
    try:
        result = subprocess.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", cmd], capture_output=True, text=True, timeout=10)
