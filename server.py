...
from db import (
    ...
    log_security_event, get_security_logs, read_settings, write_settings, init_settings
)
            ...
            for dev in current_devices:
                ...
                if serial not in known_serials:
                    ...
                    if not is_serial_whitelisted(serial):
                        ...
                        mitigation_active = system_settings.get("mitigation_enabled", True)
                        ...
                        if unlocked_state:
                            ...
                            log_security_event("Skipped mitigation for " + serial + " (Mitigation globally disabled in settings)")
                            print("[BYPASS] Skipped mitigation. Active defense is turned off.")
...
class USBSentryHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_auth_lock(self):
        ...
        try:
            UNLOCK_EXPIRE_TIME = 0.0
            log_security_event("System manually locked early by Administrator.")
            response_data = json.dumps({"status": "success"}).encode('utf-8')
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
            self.send_header('Content-Length', len(response_data))
            ...
            self.send_error_response(500, "Internal lockout error")
    ...
    def handle_get_settings(self):
        try:
            ...
            self.send_error_response(500, "Settings reading error")
    ...
    def handle_post_settings(self):
        try:
            ...
            mitigation_enabled = payload.get("mitigation_enabled", True)
            poll_interval = int(payload.get("poll_interval", 2000))
            ...
            if write_settings(new_settings):
                ...
                self.send_error_response(500, "Settings writing failure")
        except Exception:
            self.send_error_response(400, "Malformed settings payload")
    ...
    def handle_password_change(self):
        try:
            ...
            if not verify_master_password(old_password):
                self.send_error_response(401, "Invalid Old Password")
                return
            
            if not new_password or len(new_password) < 4:
                self.send_error_response(400, "New Password too short")
                return
            
            if setup_master_password(new_password):
                response_data = json.dumps({"status": "success"}).encode('utf-8')
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
                self.send_header('Content-Length', len(response_data))
                ...
                self.send_error_response(500, "Database password update error")
        except Exception:
            self.send_error_response(400, "Malformed password payload")
